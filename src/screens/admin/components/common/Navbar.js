...
const Navbar = () => {
  ...
  const onScroll = () =>{
    dispatch(startScroll(window.scrollY || window.pageYOffset > 30 ? true : false))
  }
  useEffect(() => {
  ...
  return (
    <header className=
    {
      scrollFunc?'fixed top-0 right-0 w-full backdrop-blur-md border-b px-12 flex justify-between transition-all duration-300 ease-in-out h-[70px] '
        ...
        <div className='flex flex-col'>
          <div className='p-4 relative'>
            <div className='flex justify-between'>
              ...
              <img src={process.env.PUBLIC_URL+"/img/close.svg"}className="cursor-pointer absolute top-0 right-0" alt="닫기"
              onClick={onClickShowAccount}
              />
            </div>
          ...
          onClick={()=>navigate("/admin/signin")}>
            ...
            <img src={process.env.PUBLIC_URL+"/img/arrow_next.svg"}className="cursor-pointer" alt="right_arrow"/>
          </div>
